...
export const DoctorProfileModal: React.FC<Props> = ({ children, doctor, setShowModal }: Props) => {
  const { medicalSpecialities } = useFetchMedicalSpecialities();
  const { hospital } = useFetchHospital(
    doctor.hospital_id && doctor.hospital_id !== '' ? doctor.hospital_id : undefined
  );
    ...
    <Modal setShowModal={setShowModal} className="lg:w-[644px]">
      <div className="px-6 py-6 lg:px-20">
        ...
        <div className="mt-10">
          ...
          {!['STUDENT'].includes(doctor.speciality_1) && hospital && (
            <div className="mt-6">
          ...
          <div className="mt-6 flex">
            ...
            <DoctorProfileModalLabelAndValue label="対応可能な科" className="flex-1">
              {doctor.speciality_2 && getMedicalSpecialityName(doctor.speciality_2) && (
                <div>{getMedicalSpecialityName(doctor.speciality_2)}</div>
              )}
              {doctor.speciality_3 && getMedicalSpecialityName(doctor.speciality_3) && (
                <div>{getMedicalSpecialityName(doctor.speciality_3)}</div>
              )}
              {doctor.speciality_4 && getMedicalSpecialityName(doctor.speciality_4) && (
                <div>{getMedicalSpecialityName(doctor.speciality_4)}</div>
              )}
            </DoctorProfileModalLabelAndValue>
          </div>
          <DoctorProfileModalLabelAndValue label="医師資格取得年" className="mt-6">
          ...
          <DoctorProfileModalLabelAndValue label="特によく診てきた疾患や領域" className="mt-6">
            {doctor.expertise}
          </DoctorProfileModalLabelAndValue>
